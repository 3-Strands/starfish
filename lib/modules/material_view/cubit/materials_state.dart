...
class MaterialsState {
  ...
  MaterialsPageView get materialsToShow {
    ...
    if (_selectedLanguages.isNotEmpty) {
      materials = materials
          .where((material) =>
              material.languageIds
                  .toSet()
                  .intersection(_selectedLanguages)
                  .length >
              0)
          .toList();
    }
    ...
    if (_selectedTopics.isNotEmpty) {
      materials = materials
          .where((material) =>
              material.topics.toSet().intersection(_selectedTopics).length > 0)
          .toList();
    }
    ...
    var materialWithStatus = materials
        .map((material) => MaterialWithAssignedStatus(
              material: material,
              status: _assignedMaterialStatus(material),
              isAssignedToGroupWithLeaderRole:
                  _materialIsAssignedToGroupWithLeaderRole(material),
            ))
        .toList();

    if (_actions != MaterialFilter.NO_FILTER_APPLIED) {
      materialWithStatus =
          materialWithStatus.where(_materialPassesActionFilter).toList();
    }

    final numberToTake = _pagesToShow * itemsPerPage;
    if (numberToTake < materialWithStatus.length) {
      return MaterialsPageView(
          materialWithStatus.take(numberToTake).toList(), true);
    }
    ...
    switch (_actions) {
      ...
      case MaterialFilter.ASSIGNED_AND_INCOMPLETED:
        return material.status == ActionStatus.NOT_DONE ||
            material.status == ActionStatus.OVERDUE;
  ...
  ActionStatus? _assignedMaterialStatus(Material material) {
    return _relatedMaterials.materialsAssignedToMe[material.id];
  }

  bool _materialIsAssignedToGroupWithLeaderRole(Material material) {
    return _relatedMaterials.materialsAssignedToGroupWithLeaderRole
